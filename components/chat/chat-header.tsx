...
import { ProfileCard } from "@/components/matchmaker/profiles-chat-cards";
...
function PureChatHeader({
  ...
  const openSavedMatches = async () => {
    ...
    try {
      ...
      if (matches.length > 0) {
        ...
        setSavedMatches(readSavedMatchesFromLocalStorage());
      }
    } catch {
      setSavedMatches(readSavedMatchesFromLocalStorage());
    } finally {
  ...
  const removeSavedMatch = async (id: string) => {
    ...
    const local = readSavedMatchesFromLocalStorage().filter(
      (match) => match.id !== id
    );
    writeSavedMatchesToLocalStorage(local);
